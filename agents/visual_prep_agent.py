...
def prepare_tag_breakdown_chart (agg :Dict [str ,Any ])->Dict [str ,Any ]:
    ...
    colors =[
    '#6366F1','#10B981','#FB923C','#EF4444','#60A5FA','#A78BFA','#F59E0B','#6EE7B7'
    ]
    return {
    'type':'doughnut',
    'data':{
...
def prepare_all (agg :Dict [str ,Any ])->Dict [str ,Any ]:
    return {
    'monthly_spend':prepare_monthly_spend_chart (agg ),
    'cumulative_spend':prepare_cumulative_spend_chart (agg ),
    'tag_breakdown':prepare_tag_breakdown_chart (agg ),
    'payment_mode':prepare_payment_mode_chart (agg ),
    'created_history':prepare_created_history_chart (agg ),
    'raw':agg ,
    }
    ...
    try :
        ...
        for b in bills :
            ...
            if ndt .date ()>=(now .date ()-timedelta (days =7 ))and ndt .date ()<=(now .date ()+timedelta (days =90 )):
                ...
                'name':getattr (b ,'name','')[:40 ],
                'due_date':ndt .isoformat (),
                'amount':(getattr (b ,'amount_cents',0 )or 0 )/100.0 ,
