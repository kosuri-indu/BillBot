...
def generate_narration (agg :Dict [str ,Any ],user_id :str =None )->Dict [str ,Any ]:
    safe_payload ={
    'monthly_labels':agg ['monthly']['labels'][-6 :],
    'monthly_values':agg ['monthly']['data'][-6 :],
    'top_tags':agg ['tag_breakdown']['labels'][:5 ],
    'top_tag_values':agg ['tag_breakdown']['data'][:5 ],
    }
    bullets =[]
    ...
    if genai is not None and os .environ .get ('GEMINI_API_KEY'):
        try :
            client =genai .Client ()
            prompt =(
            'You are a helpful assistant that summarizes financial metrics.\n'
            f'Here is aggregated data (last 6 months labels): {safe_payload["monthly_labels"]}\n'
            f'Here are totals: {safe_payload["monthly_values"]}\n'
            f'Top tags: {safe_payload["top_tags"]} with values {safe_payload["top_tag_values"]}\n'
            'Provide a short (1-2 sentence) human-friendly insight about recent spending trends. '
            'Be concise and avoid guessing beyond given numbers.'
            )
