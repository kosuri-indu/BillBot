...
def aggregate_user_data(user_id: str, months: int=12) -> Dict[str, Any]:
    ...
    if _USE_SUPABASE and _supabase:
        ...
        for b in bills:
            records.append({'id': getattr(b, 'id', None), 'name': getattr(b, 'name', None), 'description': getattr(b, 'description', None), 'tag': getattr(b, 'tag', None), 'payment_mode': getattr(b, 'payment_mode', None), 'amount_cents': getattr(b, 'amount_cents', 0), 'last_paid': getattr(b, 'last_paid', None).isoformat() if getattr(b, 'last_paid', None) else None, 'created_at': getattr(b, 'created_at', None).isoformat() if getattr(b, 'created_at', None) else None})
    for r in records:
        lp = r.get('last_paid')
        ca = r.get('created_at')
        date = None
    ...
    monthly = {'labels': list(totals_by_month.keys()), 'data': [round(v, 2) for v in totals_by_month.values()], 'counts': [counts_by_month[k] for k in totals_by_month.keys()]}
    tag_breakdown = {'labels': list(tag_totals.keys()), 'data': [round(v, 2) for v in tag_totals.values()]}
    payment_mode_breakdown = {'labels': list(payment_mode_totals.keys()), 'data': [round(v, 2) for v in payment_mode_totals.values()]}
    created_history = {'labels': list(created_counts.keys()), 'data': [created_counts[k] for k in created_counts.keys()]}
    payload = {'generated_at': datetime.utcnow().isoformat(), 'monthly': monthly, 'tag_breakdown': tag_breakdown, 'payment_mode_breakdown': payment_mode_breakdown, 'created_history': created_history}
    try:
